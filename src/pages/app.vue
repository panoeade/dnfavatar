  ...
  setup() {
    ...
    const images = computed(() => {
      ...
      for (const part in store.parts) {
        ...
        array.forEach((e) => (e.url = assetUrl(`/image/${professionName}/${p}/${e.name}`)));
        images.push(...array);
      }
    ...
    function style(item: Dress) {
      ...
      if (index > -1) {
        ...
        return {
          backgroundImage: `url("${assetUrl(`/icon/${item.profession}/${item.part}.png`)}")`,
                    ...
                    {renderList(store.parts, (value, part, index) => (
                      ...
                      >
                        {validateCode(value.code)
                          ? (
                              <img
                                class="m-0 h-7 w-7 flex select-none duration-100"
                                draggable="false"
                                src={assetUrl(`/icon/${store.currentProfessionName}/${part}/${value.code}.png`)}
                                title={label(value)}
                                onError={error}
                              />
                            )
